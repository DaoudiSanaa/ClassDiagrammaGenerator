...
public class Labeled extends JPanel {
	...
	public Labeled( int size,ClassParser cl) {
	    ...
	    String h1=new String(cl.newClass.getVisibility()+" class "+cl.getSimpleName() +""+cl.newClass.getSuperclass()+" implements "); 
	    ...
	    for(int i = 0;i<cl.newClass.getFields().size();i++) {  
		    JLabel j=new JLabel(cl.newClass.getFields().get(i).getVisibility()+" "+cl.newClass.getFields().get(i).getType()+" "+cl.newClass.getFields().get(i).getName()+" ;") ;
		    add(j);
	    }
	    add(new JLabel(" "));
	    ...
	    for(int i=0;i<cl.newClass.getConstructors().size();i++) {
	    	 String h2=(" ");
	    	 h2+=cl.newClass.getConstructors().get(i).getVisibility()+" "+cl.newClass.getConstructors().get(i).getName();
	    	 h2+=" ( ";
			   for(int j=0;j<cl.newClass.getConstructors().get(i).getParameters().size();j++) {
				   if(j<cl.newClass.getConstructors().get(i).getParameters().size()-1)
					   h2+=cl.newClass.getConstructors().get(i).getParameters().get(j).getType()+" "+cl.newClass.getConstructors().get(i).getParameters().get(j).getName()+",";
				   else 
					   h2+=cl.newClass.getConstructors().get(i).getParameters().get(j).getType()+" "+cl.newClass.getConstructors().get(i).getParameters().get(j).getName();
			   }
			   h2+=" ) ";
